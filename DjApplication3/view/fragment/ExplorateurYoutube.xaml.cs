...
namespace DjApplication3.view.fragment
{
    ...
    public partial class ExplorateurYoutube : System.Windows.Controls.UserControl
    {
        ...
        private void ViewModel_TacheSearch(object? sender, List<Musique> e)
        {
            ...
            {
                Musique musiqueTmp = new Musique(Path.Combine(rootFolder, $"{musique.title} ({musique.author}).mp3"), musique.title, musique.author);
    ...
    public class MusiqueColonne
    {
        ...
        public string Dl { 
            set {  dl = value; }
            get {
                string url = Path.Combine(ExplorateurYoutube.rootFolder, $"{musique.title} ({musique.author}).mp3");
                if (File.Exists(url))
                {
                    return "✔️";
                }
